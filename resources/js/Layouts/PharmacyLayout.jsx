...
export default function PharmacyLayout({ header, children }) {
    ...
    <div className="min-h-screen bg-slate-100 pb-20">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 justify-between items-center">
            <div className="flex items-center">
              <Link href="/">
                <ApplicationLogoW className="block h-9 w-auto fill-current text-white" />
              </Link>
              <div className="hidden sm:ml-10 sm:flex space-x-8">
                <NavLinkW href="/pharmacy/dashboard" active={route().current('pharmacy.dashboard')}>
                  <div className="flex items-center gap-1">
                    <FaHome />
                    Dashboard
                  </div>
                </NavLinkW>
                <NavLinkW href={route('pharmacy.prescriptions.create')} active={route().current('pharmacy.prescriptions.create')}>
                  <div className="flex items-center gap-1">
                    <FaPills />
                    Record Prescription
                  </div>
                </NavLinkW>
                <NavLinkW href={route('pharmacy.prescriptions.update.create')} active={route().current('pharmacy.prescriptions.update.create')}>
                  <div className="flex items-center gap-1">
                    <FaPills />
                    Update Prescription
                  </div>
                </NavLinkW>
                <NavLinkW href={route('pharmacy.prescriptions.log')} active={route().current('pharmacy.prescriptions.log')}>
                  <div className="flex items-center gap-1">
                    <FaClipboardCheck />
                    Prescription Log
                  </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-slate-50 border-t border-gray-200 shadow-md sm:hidden rounded-t-xl">
        <div className="flex justify-around items-center py-2">
          <Link href="/pharmacy/dashboard" className="flex flex-col items-center text-cyan-800 hover:text-cyan-950">
            <HiOutlineHome className="h-6 w-6 mb-0.5" />
            <span className="text-xs">Home</span>
          </Link>

          <Link href={route('pharmacy.prescriptions.create')} className="flex flex-col items-center text-cyan-800 hover:text-cyan-950">
          ...
          <Link href={route('pharmacy.prescriptions.log')} className="flex flex-col items-center text-cyan-800 hover:text-cyan-950">
