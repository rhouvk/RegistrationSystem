...
import { FaFilePdf, FaFilter, FaTimes } from 'react-icons/fa';

export default function DownloadSalesLogPDF({ data = [], storeName = 'My Store' }) {
  ...
  const generatePDF = () => {
    ...
    img.onload = () => {
      doc.addImage(img, 'PNG', 14, 10, 12, 12); // Logo (same as prescription log)
  
      doc.setFontSize(14);
      doc.setTextColor(33, 37, 41);
      doc.text("PWD NA 'TO – Sales Log", 30, 17); // EXACT match, just changed log type
  
      doc.setFontSize(10);
      doc.setTextColor(100);
      doc.text(`Store: ${storeName}`, 195, 17, { align: 'right' }); // Top right 1
      doc.text(`Generated: ${dateGenerated}`, 195, 21, { align: 'right' }); // Top right 2
  
      const tableData = filteredData.map((s) => [
        new Date(s.date_of_sale).toLocaleDateString(),
        s.purchased_by,
        s.item_name,
        s.quantity,
      ...
      autoTable(doc, {
        head: [['Date', 'Purchased By', 'Item', 'Qty', 'Amount']],
        body: tableData,
        startY: 26,
        styles: { fontSize: 8 },
