...
class PWDBarcodeController extends Controller
{
    public function generate(Request $request)
    {
        $pwd = PWDRegistration::with(['disabilityType', 'municipality', 'user'])->where('pwdNumber', $request->input('id'))->first();
        ...
        if (file_exists($sigPath)) {
            ...
            $transparent = imagecolorallocatealpha($sigResized, 0, 0, 0, 127);
            imagefill($sigResized, 0, 0, $transparent);
        ...
        imagettftext($canvas, $fs, 0, 144, 244, $black, $fontPath, strtoupper(optional($pwd->user)->name ?? 'PWD NAME'));
        imagettftext($canvas, $fs, 0, 270, 297, $black, $fontPath, strtoupper(optional($pwd->disabilityType)->name ?? 'DISABILITY'));
